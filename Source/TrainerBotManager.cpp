...
void TrainerBotManager::BotSetup() {
    ...
    TheTrainer->dataManager.ConfigureDataName(38, "Distance Left");
    TheTrainer->dataManager.ConfigureDataName(39, "Angle Velocity Left");
...
void TrainerBotManager::BotCommand() {
    ...
    if (t >= 6) {
        ...
        if ((AngleVelocity[0] <= 0 && (deltaTheta[0] < 0 && deltaTheta[0] >= -1.2)) || (AngleVelocity[1] <= 0 && (deltaTheta[1] < 0 && deltaTheta[1] >= -1.2))) {
            MotorTorque[0] = TorqueWanted[0] + 5 + f_ang_v_0 * Cm + f_ang_a_0 * Jm; //5 is the torque needed to start the motor
            ...
            MotorTorque[0] = TorqueWanted[0] + 8 + f_ang_v_0 * Cm + f_ang_a_0 * Jm; //8 is the torque needed to start the motor
        ...
        MotorTorque[1] = TorqueWanted[1] / (1 + pow(e, -0.8 * (t - 1))); //Gain 0.8 biais 5 to match acceleration from left side and right side 
    }
...
void TrainerBotManager::BotSaveData() {
    ...
    TheTrainer->dataManager.AddData(38, DistanceRelativeValueUsingCounter[0]); // Relative Angle Velocity Right
    TheTrainer->dataManager.AddData(39, AngleVelocity[0]); // Relative Angle Velocity Right
}
