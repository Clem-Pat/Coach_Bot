                ...
                if (ImPlot::BeginPlot("Data", Abscissa.c_str(), "")) {
                    ...
                    for (auto& point : listPointsToPlot) {
                        if (point.index == i) {
                            ...
                            if (point.type == "lowerBound" || point.type == "upperBound") {

                                //Either we choose to draw a line from the 
                                ...
                                cout << "Faire une ligne horizontale" << endl;
							}
                        }
                    }
                    ...
                    if (ImGui::GetIO().KeyCtrl && ImGui::IsMouseClicked(ImGuiMouseButton_Left) && !HasAlreadyClickedOnPlot) {
                        ...
                        else if (listPointsToPlot.size() == 1) {
...
void TrainerApp::CreateUsersInfoWindow() {
    ...
    if (ImGui::SliderInt("Input Motor Torque (Kg)", &TheTrainer->botManager.TotalWeightWanted, 0, TheTrainer->botManager.MaxOutputMotorTorque)) {
        trapeze.update();
    }
    //ImGui::Spacing();
    ...
    ImGui::Checkbox("Want to use load profile ?", &TheTrainer->UsingLoadProfile);
    ImGui::Dummy(ImVec2(0.0f, 20.0f));
    ...
    if (!this->TheTrainer->WantToStartBot && !this->TheTrainer->BotWorkingInProgress) {
        ...
        //}
        if (!this->TheTrainer->predictor.WantToAnalyzeData){
            ImGui::SameLine();
            if (ImGui::Button("Analyze repetitions")) {
                this->TheTrainer->predictor.WantToAnalyzeData = true;
                this->TheTrainer->UsingLoadProfile = false;
                this->TheTrainer->Want1RMExperiment = true;
...
void TrainerApp::CreateBotModeWindow() {
    ...
    ImGui::Checkbox("Take off the clutch", &TheTrainer->botManager.WantTakeOffTheClutch);
