...
void TrainerApp::ClearPlot() {
    ...
    listLinesToPlot.erase(
        std::remove_if(listLinesToPlot.begin(), listLinesToPlot.end(),
            [](const Line& line) { return line.index != 2; }),
        listLinesToPlot.end());
}
...
void TrainerApp::RenderDataInPlot(double i) {
    for (int j = 0; j < listIndexesToPlot.size(); ++j) {
        ...
        //Right click on graph to do the regression. We did not delete this obsolete option so you can uncomment it and use it if you want.
...
void TrainerApp::CreatePointOnMouseCtrlPlusClick() {
...
void TrainerApp::SubplotItemSharing() {
    ...
    if (ImPlot::BeginSubplots("##First row - Data", 1, 1, ImVec2(-1, subplotHeights[0]))) {
        ...
        if (ImPlot::BeginPlot("Data", "Time in seconds", "", ImVec2(-1, 0), plot_flags)) { // PLOT WITH INDEX 0
            ImPlot::SetupAxesLimits(-15, 60, -0.5, 0.5); // Set the x range to [x1, x2] and the y range to [y1, y2]
            RenderPointsAndLineInPlot(0);
            RenderDataInPlot(0);
            SetUpQueryInPlot();
            CreatePointOnMouseCtrlPlusClick(); //Create a Point on mouse position Ctrl+click
            ImPlot::EndPlot();
        }
    ...
    if (TheTrainer->predictor.FindLastAnalyserOfType<OneRMAnalyser>() != -1){
        if (ImPlot::BeginSubplots("##Second Row One RM experiment", 1, 2, ImVec2(-1, subplotHeights[1]))) { // Adjust size as needed
            ...
            if (ImPlot::BeginPlot("One RM", "Load in kg", "Velocity", ImVec2(-1, 180), plot_flags)) { // PLOT WITH INDEX 1
            ...
            if (ImPlot::BeginPlot("Endurance", "Velocity loss in %", "Repetition in %", ImVec2(-1, 180))) { // PLOT WITH INDEX 1.1
    ...
    if (TheTrainer->UsingLoadProfile) {
        if (ImPlot::BeginPlot("Profile", "Length (m)", "Weight (kg)", ImVec2(-1, subplotHeights[2]))) { // PLOT WITH INDEX 2
...
void TrainerApp::CreateUsersInfoWindow() {
    ...
    if (ImGui::InputText("Username", usernameBuffer, IM_ARRAYSIZE(usernameBuffer), ImGuiInputTextFlags_EnterReturnsTrue)) { // This block is executed when the user presses Enter while the input field is active
        ...
        TheTrainer->UsingLoadProfile = false; // We change username, so we need to re-do the set of reference to calculate the MaximumLengthPulledByUser and the MinimumLengthPulledByUser
    }
    ImGui::SetNextItemWidth(tabWidth * 0.5f);
    ...
    if (ImGui::InputText("Type of exercise", typeOfExercise, IM_ARRAYSIZE(typeOfExercise), ImGuiInputTextFlags_EnterReturnsTrue)) { // This block is executed when the user presses Enter while the input field is active
        ...
        TheTrainer->UsingLoadProfile = false; // We change username, so we need to re-do the set of reference to calculate the MaximumLengthPulledByUser
    }
    ...
    if (ImGui::Checkbox("Want to use load profile ?", &TheTrainer->UsingLoadProfile)) {
        if (TheTrainer->UsingLoadProfile){
            ...
            double acquiredMinHeight = TheTrainer->predictor.sessionAnalysers[0].get()->getReferencedMinHeight();
            if (acquiredMaxHeight == 0) {
                ...
                TheTrainer->MinimumLengthPulledByUser = acquiredMinHeight;
                ...
                AddRobotConsoleMessage("We analyzed your repetitions. Max Height is " + std::to_string(acquiredMaxHeight) + ". Min Height is " + std::to_string(acquiredMinHeight));
            }
        }
    }
    ImGui::Dummy(ImVec2(0.0f, 20.0f));
...
void TrainerApp::CreatePlotWindow() {
    if (ImGui::Begin("Output Data plot")) {
        ...
        if (TheTrainer->predictor.FindLastAnalyserOfType<OneRMAnalyser>() == -1 && TheTrainer->dataManager.FindInDataBase(TheTrainer->Username, TheTrainer->TypeOfExercise, "NULL", "1", "usedForOneRM") != "NULL") { //If we found a previous One RM experiment saved in the database and if we have not already done an One RM experiment today, we can upload the last one
            ImGui::SameLine();
            if (ImGui::Button("Upload last One RM Experiment")) { TheTrainer->dataManager.UploadLastOneRMExperimentFromDataBase(TheTrainer->Username, TheTrainer->TypeOfExercise); }
        }
        if (TheTrainer->predictor.FindLastAnalyserOfType<OneRMAnalyser>() != -1 && TheTrainer->predictor.FindLastAnalyserOfType<SetDataSaver>() != -1) { // If we have done a One RM experiment, we can compare the last set done with the One RM experiment results
			ImGui::SameLine();
            ImGui::Checkbox("Plot last set in One RM experiment", &PlotLastSetInOneRMGraphToCompareThem);
            if (PlotLastSetInOneRMGraphToCompareThem) { TheTrainer->predictor.CompareLastSetWithOneRM(); }
            else { Unplot("Compare set with One RM"); Unplot("Compare set with Endurance"); }
		}
        if (TheTrainer->predictor.MaxNumberRepsEstimated != -1){  // progress bar of the set computed zith the Endurance Experiment
...
int TrainerApp::HasLineOfType(std::string type){
    for (int i = 0; i < listLinesToPlot.size(); i++) {
        if (listLinesToPlot[i].type == type) {
            return i;
        }
    }
    return -1;
...
void TrainerApp::AddVerticalLineToPlot(double xValue, std::string type, int index) {
    //If the line is a vertical line for left and right distance in profile plot, if it already exists we update it. So we first delete it ...
    if (TheTrainer->UsingLoadProfile && (type == "Left Distance" || type == "Right Distance" || type == "Maximum length pulled by user" || type == "Minimum length pulled by user")) {
        if (HasLineOfType(type) != -1) {
            listLinesToPlot.erase(listLinesToPlot.begin() + HasLineOfType(type));
        }
    }
    listLinesToPlot.push_back(VerticalLine(this, xValue, type, index)); //... then we add it
}
