...
void TrainerApp::CompareLastSetWithOneRM() {
    OneRMAnalyzer* oneRMAnalyzer = dynamic_cast<OneRMAnalyzer*>(TheTrainer->predictor.seriesAnalyzers[TheTrainer->predictor.FindLastAnalyzerOfType<OneRMAnalyzer>()].get());
    SetDataSaver* lastSet = dynamic_cast<SetDataSaver*>(TheTrainer->predictor.seriesAnalyzers[TheTrainer->predictor.FindLastAnalyzerOfType<SetDataSaver>()].get());
    ...
    if (lastSet->closed){
        ...
        TheTrainer->app.AddRobotConsoleMessage(("The minimum velocity of last set is " + std::to_string(lastSet->minMaxVelocities) + ". The difference with the One RM velocity value is : " + std::to_string(lastSet->minMaxVelocities - oneRMAnalyzer->OneRMVelocityValue)).c_str());
        TheTrainer->app.AddRobotConsoleMessage(("The load lifted is " + std::to_string(lastSet->TotalWeightWanted * 100 / oneRMAnalyzer->OneRMValue) + " % of One RM load").c_str());
        TheTrainer->app.AddRobotConsoleMessage(("The load lifted is " + std::to_string(lastSet->TotalWeightWanted * 100 / oneRMAnalyzer->OneRMValue) + " % of One RM load").c_str());
    }
}
...
void TrainerApp::AllowDeletingPointByRightClickingOnIt(int index) {
    ...
    if (ImGui::BeginPopup("Delete Point")) {
        if (index == 1){
            if (ImGui::MenuItem("Delete")){
                auto it = TheTrainer->predictor.FindLastAnalyzerOfType<OneRMAnalyzer>();
                if (it != -1) {
                    OneRMAnalyzer* oneRMAnalyzer = dynamic_cast<OneRMAnalyzer*>(TheTrainer->predictor.seriesAnalyzers[it].get());
                    std::ostringstream oss;
                    oss << "deleted OneRM point {"
                        << std::fixed << std::setprecision(5)
                        << it << ", "
                        << j_toRemember << ", "
                        << oneRMAnalyzer->OneRMExperimentLoads[j_toRemember] << ", "
                        << oneRMAnalyzer->OneRMExperimentVelocities[j_toRemember] << ", "
                        << oneRMAnalyzer->EnduranceExperimentVelocitiesLoss[j_toRemember] << ", "
                        << oneRMAnalyzer->EnduranceExperimentRepetitionRatio[j_toRemember] << "}";

                    LastActionDone.push_back(oss.str());
                    oneRMAnalyzer->OneRMExperimentLoads.erase(oneRMAnalyzer->OneRMExperimentLoads.begin() + j_toRemember);
                    oneRMAnalyzer->OneRMExperimentVelocities.erase(oneRMAnalyzer->OneRMExperimentVelocities.begin() + j_toRemember);
                    oneRMAnalyzer->EnduranceExperimentVelocitiesLoss.erase(oneRMAnalyzer->EnduranceExperimentVelocitiesLoss.begin() + j_toRemember);
                    oneRMAnalyzer->EnduranceExperimentRepetitionRatio.erase(oneRMAnalyzer->EnduranceExperimentRepetitionRatio.begin() + j_toRemember);
                    oneRMAnalyzer->UpdateAndPlotOneRMExperiment();
                }
                ImGui::CloseCurrentPopup();
            }
            ImGui::EndPopup();
...
void TrainerApp::UndoLastAction(){
    if (LastActionDone.size() > 0){
        ...
        if (action.find("deleted OneRM point") != std::string::npos) { //If the last action done is to delete a point in One RM experiment, we add it back to the experiment
            ...
            int indexOfOneRMAnalyzer = (int)values[0];
            int j_toInsert = (int)values[1];
		    OneRMAnalyzer* oneRMAnalyzer = dynamic_cast<OneRMAnalyzer*>(TheTrainer->predictor.seriesAnalyzers[indexOfOneRMAnalyzer].get());
            oneRMAnalyzer->OneRMExperimentLoads.insert(oneRMAnalyzer->OneRMExperimentLoads.begin() + j_toInsert, values[2]);
            oneRMAnalyzer->OneRMExperimentVelocities.insert(oneRMAnalyzer->OneRMExperimentVelocities.begin() + j_toInsert, values[3]);
            oneRMAnalyzer->EnduranceExperimentVelocitiesLoss.insert(oneRMAnalyzer->EnduranceExperimentVelocitiesLoss.begin() + j_toInsert, values[4]);
            oneRMAnalyzer->EnduranceExperimentRepetitionRatio.insert(oneRMAnalyzer->EnduranceExperimentRepetitionRatio.begin() + j_toInsert, values[5]);
            oneRMAnalyzer->UpdateAndPlotOneRMExperiment();
	    }
