...
void TrainerApp::CreateBotModeWindow() {
    ...
    if (TheTrainer->botManager.UsingContecAIO){         //If the ContecAIO is plugged in, it means that we are using the old robot that has clutches. So we allow the user to change them. 
    ...
    ImGui::Dummy(ImVec2(0.0f, 20.0f));
    ImGui::Checkbox("Experiment mode", &TheTrainer->ExperimentMode);
    ImGui::End();
}
