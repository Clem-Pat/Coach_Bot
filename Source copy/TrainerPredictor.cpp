...
void TrainerPredictor::EraseGraphToDoRegressionOn(int indexOfAbscissaData, int indexOfDataToDoRegressionOn, bool WantUnplot) {
    std::string label = "regression " + TheTrainer->app.listLabelsToPlot[indexOfDataToDoRegressionOn];
    if (WantUnplot) { TheTrainer->app.Unplot(label); }
    ...
    for (int i = 0; i < indexesOfAbscissaData.size(); i++)
    {
        ...
        if (y.size() > 2) {
            if (std::to_string(y[0]) != "-nan(ind)"){
                ...
                if (TheTrainer->app.listLabelsToPlot[indexesOfDataToDoRegressionOn[i]] == "Max velocity during reps") { //We are doing the regression of the One RM experiment 
                    isPolynomial = false;
                ...
                //cout << line << endl;
                //if (TheTrainer->app.listLabelsToPlot[indexesOfDataToDoRegressionOn[i]] == "Max velocity during reps") { // After plotting the OneRM regression...
                //    EraseGraphToDoRegressionOn(indexesOfAbscissaData[i], indexesOfDataToDoRegressionOn[i], false); // ... We want to calculate the Regression only once. We do not want it to be updated.
                //}
            }
            else {
