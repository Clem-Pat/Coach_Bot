    ...
    if (TheTrainer->predictor.FindLastAnalyzerOfType<SetDataSaver>() != -1) {
        ...
        for (int i = 0; i < seriesAnalyzers.size() - 1; i++) {
            if (seriesAnalyzers[i].get()->name == "SetDataSaver") {
                ...
                if (analyzer->Username == setSaver->Username && analyzer->TypeOfExercise == setSaver->TypeOfExercise && analyzer->TotalWeightWanted == setSaver->TotalWeightWanted) {
    ...
    if (indexLastSetDataSaverAnalyzer != -1) {
        ...
        if (!setAnalyzer->closed){
            ...
            if (maxHeightElementIt != setAnalyzer->HeightOfSet[0].end()) { // Ensure the iterator is not the end iterator
                ...
                std::vector<std::vector<double>> result = findPeaksBoundaries(setAnalyzer->HeightOfSet[0], peaksValue);
                if (result[0].size() > 0) { //We have done more than one rep we can proceed analyzing
                    ...
                    TheTrainer->app.AddHorizontalLineToPlot(peaksValue);
                    result = findPeaksBoundaries(setAnalyzer->HeightOfSet[0], peaksValue);
...
void TrainerPredictor::LepleyExperiment(vector<double> t_list) {
    ...
    if (WantStopLepleyExperiment) {
        ...
        if (FindLastAnalyzerOfType<CounterAnalyzer>() != -1) {
            CounterAnalyzer* LastCounterAnalyzer = dynamic_cast<CounterAnalyzer*>(seriesAnalyzers[FindLastAnalyzerOfType<CounterAnalyzer>()].get());
            if (!LastCounterAnalyzer->closed) {
                LastCounterAnalyzer->DoMission();
                LastCounterAnalyzer->closeAnalyzer(t_list.back());
            }
        }
...
void TrainerPredictor::AnalyzeData() {
    ...
    int indexLastSetDataSaverAnalyzer = FindLastAnalyzerOfType<SetDataSaver>();
    if (indexLastSetDataSaverAnalyzer != -1) {
        if (TheTrainer->BotWorkingInProgress && (TheTrainer->Username != seriesAnalyzers[indexLastSetDataSaverAnalyzer].get()->Username || TheTrainer->TypeOfExercise != seriesAnalyzers[indexLastSetDataSaverAnalyzer].get()->TypeOfExercise || TheTrainer->botManager.TotalWeightWanted != seriesAnalyzers[indexLastSetDataSaverAnalyzer].get()->TotalWeightWanted || TheTrainer->UsingLoadProfile != seriesAnalyzers[indexLastSetDataSaverAnalyzer].get()->UsingLoadProfile)) {
