...
#include "boost/math/distributions/fisher_f.hpp"    // For Fisher F distribution
#include "boost/math/distributions.hpp"             // For Fisher F distribution
...
void eraseTheProblematicReps(SetDataSaver* lastDataSaver) {
    for (int j = 0; j < lastDataSaver->setsWithProblemsToKeep.size(); j++) {
        if (!lastDataSaver->setsWithProblemsToKeep[j]) {
            ...
            lastDataSaver->minHeights.erase(lastDataSaver->minHeights.begin() + index);
...
void TrainerPredictor::Begin() {
    sessionAnalysers.push_back(std::make_unique<InitialAnalyser>(TheTrainer, 0, 0));
}
...
void TrainerPredictor::CompareLastSetWithOneRM() {
...
double TrainerPredictor::FindOneRM(std::vector<double> X, std::vector<double> regressionCoefficients, int i) {
    ...
    if (TheTrainer->predictor.FindLastAnalyserOfType<OneRMAnalyser>() != -1) {
        ...
        if (i != -1){ //The calculation of One RM is used in the main window so we want to plot it. 
            TheTrainer->app.Plot(index, XOneRM, YOneRM, TheTrainer->app.listIndexesToPlot[indexesOfDataToDoRegressionOn[i]], OneRMLabel, "lines"); //Draw horizontal line at mean min Velocity
        }
        else {  //The calculation of One RM is used in a popup window so we want to plot it.
            TheTrainer->dataRenderer.Plot(XOneRM, YOneRM, OneRMLabel, "lines"); //Draw horizontal line at mean min velocity
        }
        ...
        if (regressionCoefficients[0] < 0 && (LastOneRManalyser->OneRMVelocityValue - regressionCoefficients[1]) / regressionCoefficients[0] > 0){
            ...
            if (i != -1) { //The calculation of One RM is used in the main window so we want to plot it.
                ...
                TheTrainer->dataRenderer.Plot(x_values, y_values, OneRMPointlabel, "scatter");
            }
            //END Conjuncture point between the regression line and the horizontal line
            LastOneRManalyser->CoefficientsOneRMRegression = regressionCoefficients;
            return (LastOneRManalyser->OneRMVelocityValue - regressionCoefficients[1]) / regressionCoefficients[0];
        }
        else {
            if (i != -1){ //We call this function from the main window app so we want to have the error message in the console
...
void TrainerPredictor::PlotRegression(std::vector<double> Xvalues, std::vector<double> coefficients, std::string subsublabel, int i, bool shallWeKeepThePlotEvenAfterAClearPlot) {
    ...
    else { //We call this function from a popup window so we want to plot it.
		TheTrainer->dataRenderer.Plot(X, Y, type + " regression " + subsublabel, "lines");
    }
}
        ...
        if (y.size() > 2) {
            if (std::to_string(y[0]) != "-nan(ind)"){
                ...
                if (TheTrainer->app.listLabelsToPlot[indexesOfDataToDoRegressionOn[i]] == "Max velocity during reps") {
                    ...
                    //TheTrainer->app.Plot(index, newX, newY, 1, "Mean Values of velocity (with exponential decreasing weights)", "scatter"); //Draw horizontal line at 0.2
...
void TrainerPredictor::BotStartedSet(vector<double> t_list) {
    ...
    dynamic_cast<InitialAnalyser*>(sessionAnalysers[0].get())->numberOfSets++;
...
void TrainerPredictor::BotFinishedSet(vector<double> t_list) {
    ...
    dynamic_cast<InitialAnalyser*>(sessionAnalysers[0].get())->update(true);
    ...
    if (TheTrainer->ExperimentMode) TheTrainer->dataManager.SaveSetInFile();
}
                    ...
                    if (result[0].size() != 0 && result[1].size() != 0){
                        ...
                        for (int i = 0; i < setAnalyser->indexesEnteringARep.size(); i++) {
                            ...
                            if (heightOfRep.size() != 0 && velocityOfRep.size() != 0 && accelerationOfRep.size() != 0) {
                                //Get the min Height of the rep
                                auto minHeightOfRepElementIt = std::min_element(heightOfRep.begin(), heightOfRep.end());
                                double minHeightOfRep = 0;
                                if (minHeightOfRepElementIt != heightOfRep.end()) { // Ensure the iterator is not the end iterator
                                    minHeightOfRep = *minHeightOfRepElementIt; // Dereference the iterator to get the value
                                }
                                setAnalyser->minHeights.push_back(minHeightOfRep);
                        ...
                        if (setAnalyser->maxVelocities.size() != 0) {
                            dynamic_cast<InitialAnalyser*>(sessionAnalysers[0].get())->minMaxVelocitiesOfSets.push_back(*std::min_element(setAnalyser->maxVelocities.begin(), setAnalyser->maxVelocities.end()));
                        }
...
void TrainerPredictor::AnalyzeData() {
    ...
    //Update the initial analyser
    dynamic_cast<InitialAnalyser*>(sessionAnalysers[0].get())->update();
