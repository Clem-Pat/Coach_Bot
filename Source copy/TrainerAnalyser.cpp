...
double TrainerAnalyser::getReferencedMinHeight() {
	std::string username = TheTrainer->Username;
	std::string typeOfExercise = TheTrainer->TypeOfExercise;
	std::vector<double> referencedMinHeights = std::vector<double>();
	for (auto& analyser : TheTrainer->predictor.sessionAnalysers) {
		if (dynamic_cast<SetDataSaver*>(analyser.get())) {
			SetDataSaver* setAnalyser = dynamic_cast<SetDataSaver*>(analyser.get());
			if (setAnalyser->Username == username && setAnalyser->TypeOfExercise == typeOfExercise) {
				if (referencedMinHeights.size() > 5) {
					break;
				}
				else {
					for (double minHeight : setAnalyser->minHeights) {
						referencedMinHeights.push_back(minHeight);
					}
				}
			}
		}
	}
	if (referencedMinHeights.size() == 0) return 0; // We never acquired heights so we return the default value
	return std::accumulate(referencedMinHeights.begin(), referencedMinHeights.end(), 0.0) / referencedMinHeights.size();
}
...
void OneRMAnalyser::UpdateAndPlotOneRMExperiment() {
	...
	if (TheTrainer->predictor.WantStopOneRMExperiment || Date != TheTrainer->dataManager.GetTodaysDate()) {
